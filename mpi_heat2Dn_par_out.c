...
int main (int argc, char *argv[]) {
  ...
  int rec;                                /* File opening error handle */
  int prev_output;                        /* Offset for printing in file */
  char space=' ',newl='\n';               /* Characters to use with MPI printing functions*/
  MPI_File file;                          /* File pointer*/
  int output_size                         /* Size in bytes of a process' output*/
  int row_size;                           /* Size in bytes of a single process' row output*/
...
/* Start parallel output */
  ...
  prev_output = output_size * coord[0]* cart_dims[1];

  rec = MPI_File_open(MPI_COMM_WORLD,"./mpiheatout", MPI_MODE_CREATE | MPI_MODE_WRONLY , MPI_INFO_NULL, &file);
